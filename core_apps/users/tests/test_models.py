...
def test_create_super_user_with_no_email(user_factory):
    ...
    assert str(err.value) == "Superusers must have an email address."
...
@pytest.mark.django_db
def test_create_super_user_is_not_staff(user_factory):
    with pytest.raises(ValueError) as err:
        user_factory.create(is_superuser = True , is_staff = False)
    assert str(err.value) == "Superuser must have is_staff=True."

@pytest.mark.django_db
def test_create_super_user_is_not_superuser(user_factory):
    with pytest.raises(ValueError) as err:
