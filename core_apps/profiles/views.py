...
from authores_api.settings.local import DEFAULT_FROM_EMAIL
...
class FollowerListView(APIView):
    ...
    def get(self,request,format=None):
        try:
            ...
            return Response(formated_response , status.HTTP_200_OK)
...
class FollowingListView(APIView):
    ...
    def get(self,request,user_id,format=None):
        try:
            ...
            following_profiles = profile.fillowing.all()
...
class FollowAPIView(APIView):
    def post(self,request,user_id,format=None):
        try:
            ...
            subject = "A new user following you."
            ...
            recepient_list = [profile.user.email]
            send_mail(subject,message,from_email,recepient_list,fail_silently=True)
            return Response({
                "status_code" : status.HTTP_200_OK,
                "message" : f"You are now following {profile.user.first_name} {profile.user.last_login}"
            },status.HTTP_200_OK)
    ...
    def post(self,request,user_id,*args,**kwargs):
        ...
        if not user_profile.check_following(profile):
            formatted_response = {
                ...
                "message" : f"You cant unfollow {profile.user.first_name} {profile.user.last_name} since you are not following them in first place."
            }
